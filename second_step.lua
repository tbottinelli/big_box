...
function main(args)
    ...
    print(nparticle)
    --local npore = assert(sample.npore)
    --print(npore)
    local nfluid = nparticle - npore
    print(nfluid)
    ...
    local rest_size = edges[1][1] - Source_size
    ...
    local obstacle_group = mdsim.particle_groups.id_range({particle = particle,range={nfluid + 1,nfluid + npore},label = "allobstacle"})
    ...
    phase_space:writer({file = file_write, fields = {"position", "velocity", "species", "force", "image","mass"}, every = interval})
...
function define_args(parser)
    ...
    parser:add_argument("nknots", {type = "number", default = 801, help = "number of knots"})
